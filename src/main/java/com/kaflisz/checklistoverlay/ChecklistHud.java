...
public class ChecklistHud {
    ...
    public static void render(DrawContext context, float tickDelta) {
        ...
        if (!state.visible || state.joinedListName == null || state.entries.isEmpty()) return;
    ...
    public static void drawPanel(DrawContext context, int x, int y, boolean editMode) {
        ...
        String listTag = state.joinedListName != null ? " \u2022 " + state.joinedListName : "";
        String headerLabel = "Checklist" + listTag + " " + (state.collapsed ? "[+]" : "[-]") + (editMode ? " (drag)" : "");
        ...
        if (!state.collapsed) {
            ...
            for (ChecklistEntry entry : entries) {
                if (rowY + ROW_HEIGHT > y + HEADER_HEIGHT && rowY < y + height) {
                    ...
                    String label = entry.contributedQty + "/" + entry.targetQty + "  " + itemName;
